...
Suppose the decision of a female (with parental roots either from the GDR or the FRG and residing in reunified Germany) whether to study a STEM subject or a non-STEM subject at university can be thought of as a maximisation problem of the two-period model

\vspace{-8mm}

\begin{equation}
	\label{eq:utility}
	U = u(c_{0}) - \mathds{1}v_{i} + \beta{}u(c_{1})
\end{equation}

\noindent with $u$ being a strictly increasing and concave function, $\mathds{1}$ being an indicator function which equals one if the female $i$ is enrolled in a STEM subject and zero if she is enrolled in a non-STEM subject. $v_{i}$ is the disutility of studying a STEM subject, $\beta \in (0,1)$ is an exogenously given discount factor of future consumption that does not vary across individuals,
...
\begin{equation}
	\label{eq:c1}
	c_{1} = w_{1} + \mathds{1}(w_{1, \textnormal{ STEM}} - w_{1})
\end{equation}

\noindent is consumption after receiving the university degree and working.
For simplicity, the incomes $w_{0}$ (income as a student in period 0), $w_{1}$ (working in non-STEM in period 1), and $w_{1, \textnormal{ STEM}}$ (working in STEM in period 1) are taken as exogenous and do not vary across individuals. Further, assume that $w_{0} < w_{1} < w_{1, \textnormal{ STEM}}$ holds.

The disutility of studying a STEM subject $v_{i}$ varies across females and is assumed to be a random draw from a country-of-ancestry-specific distribution with mean $m_{k}$ where $k \in \{\textnormal{GDR, FRG}\}$ and variance $\sigma^{2}$. The cumulative distribution function (cdf for short) is denoted by $G_{k}(m_{k}, \sigma)$.
